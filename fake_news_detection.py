...
df = pd.read_csv('final_news_dataset.csv', usecols = ['title', 'content', 'publication', 'label'])
...
def extract_features(df, field, training_data, testing_data, vocabulary):
    ...
    #vectorizer = TfidfVectorizer(use_idf = True, max_df = 200, stop_words = 'english')
...
def train_SVM_model(df, field, vocab):
  ...
  return feature_tranformer, y_pred, Y_test
